...
class DataRepository:
    ...
    def get_song_lyrics(self, artist_name, song_title):
        url = (
            f"https://api.lyrics.ovh/v1/{artist_name}/"
    ...
    def add_song_from_data(self, data, album: Album):
        ...
        if not lyrics:
            return
        artist = (
            self.bot.session.query(Artist)
            .filter_by(spotify_id=data["artists"][0]["id"])
            .first()
        )
        existing_album = (
            self.bot.session.query(Album).filter_by(spotify_id=album.spotify_id).first()
        )
        song = (
        ...
        if not artist:
            artist = Artist(
                spotify_id=data["artists"][0]["id"], name=data["artists"][0]["name"]
            )
