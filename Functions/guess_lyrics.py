...
class GuessLyrics(commands.Cog):
    ...
    @app_commands.command(name='start',
        description="Start a game to guess the lyrics of a song.")
    async def start(self, interaction: discord.Interaction):
        if self.game_state.is_game_active:
            await interaction.response.send_message("Game is already active.", ephemeral=True)
            return
        try:
    ...
    async def start_by_album_url(self, interaction: discord.Interaction, url: str):
        if self.game_state.is_game_active:
            return await interaction.response.send_message("Game is already active.", ephemeral=True)
    ...
    async def hint(self, interaction: discord.Interaction):
        ...
        if interaction.channel != self.game_state.thread:
            return await interaction.response.send_message(
                "You can only get hints in the game thread.",
                ephemeral=True
            )
        ...
        await interaction.response.defer()
        if self.game_state.attempts >= self.FIRST_HINT and self.game_state.attempts < self.SECOND_HINT:
